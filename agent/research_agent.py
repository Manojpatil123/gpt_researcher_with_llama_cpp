...
class ResearchAgent:
    ...
    async def create_search_queries(self):
        ...
        try:    
            a1=result.split('query 1:')[1].split('query 2:')[0]
            a2=result.split('query 1:')[1].split('query 2:')[1]
           ...
           print(e)
           try:
            a1=result.split('[')[1].split(']')[0].split(',')[0]
            a2=result.split('[')[1].split(']')[0].split(',')[1]
        ...
        await self.websocket.send_json({"type": "logs", "output": f"🧠 I will conduct my research based on the following queries: {result1}..."})
